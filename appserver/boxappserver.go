...
func main() {
	...
	file, err := os.OpenFile("/var/log/casb/box_appserver.log", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		log.Fatal(err)
	}
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, "- Response from %s running at port %s.\n", BOX_APP_NAME, BOX_APP_PORT)
		fmt.Fprintf(w, "- Host: %s URL:%s\n", r.Host, r.URL.Path)
		fmt.Fprintf(w, "- Nonce: %s\n", nonce)
		fmt.Fprintf(w, "- Current Time is: %s.", currentTime.Format("01-02-2006 15:04:05"))
	})

	http.HandleFunc("/v2/box/notification/", func(w http.ResponseWriter, r *http.Request) {
		log.Print("BOX NOTIFICATION - Got request.")

		nonce := r.Header.Get("nonce")
		tenant := r.Header.Get(TENANT_HEADER)
		currentTime := time.Now()

		fmt.Fprintf(w, "Response from %s!! NOTIFY -  Current Time is: %s\n", BOX_APP_NAME, currentTime.Format("01-02-2006 15:04:05"))
	...
	http.HandleFunc("/v2/box/notification/check/", func(w http.ResponseWriter, r *http.Request) {
		log.Print("BOX CHECK - Got request.")

		nonce := r.Header.Get("nonce")
		tenant := r.Header.Get(TENANT_HEADER)
		currentTime := time.Now()

		fmt.Fprintf(w, "Response from %s!! CHECK Current Time is: %s\n", BOX_APP_NAME, currentTime.Format("01-02-2006 15:04:05"))
	...
	runningPort := fmt.Sprintf(":%s", BOX_APP_PORT)
